...
function loadFarmDetail(el) {
  ...
  fetch('/api/ocean-water/alert?lat=' + lat + '&lon=' + lon)
    ...
    .then(function (d) {
      ...
      // 加载 SDM 生态位预警（鱼种适宜性）
      fetch('/api/ocean/ecowarn?lat=' + lat + '&lon=' + lon + '&species=xiaohuangyu')
        .then(function (r) { return r.json(); })
        .then(function (e) {
          var box = document.getElementById('farmEcoWarn');
          if (!box) return;
          if (!e.ok) { box.innerHTML = '<div style="color:#999;">鱼种适宜性：暂无模型</div>'; return; }
          var suit = (e.suitability * 100).toFixed(0);
          var base = e.baseline != null ? (e.baseline * 100).toFixed(0) : '—';
          var ecoAlert = e.alert ? '<div class="farm-alert ' + e.alert.level + '">' + e.alert.msg + '</div>' : '<div class="farm-alert ok">鱼种适宜性正常</div>';
          box.innerHTML = '<div style="font-weight:600;margin-bottom:4px;">' + e.species + ' 适宜性（SDM 生态位）</div>'
            + '<div class="farm-metrics"><div class="fm"><span class="fm-k">当前适宜性</span><span class="fm-v">' + suit + '%</span></div>'
            + '<div class="fm"><span class="fm-k">季节基准</span><span class="fm-v">' + base + '%</span></div></div>'
            + ecoAlert;
        })
        .catch(function () { var box = document.getElementById('farmEcoWarn'); if (box) box.innerHTML = '<div style="color:#999;">鱼种适宜性：加载失败</div>'; });
    })
...
function showFarmSeason() {
  ...
  if (map._farmSeasonLand) { map.removeLayer(map._farmSeasonLand); map._farmSeasonLand = null; }
  ...
  fetch('/static/data/sdm/sdm_prediction_' + sp + '_' + season + '_cn.json')
    ...
    .then(function (d) {
      ...
      // 陆地遮罩（10m GeoJSON）
      if (!map._farmLandLoaded) {
        fetch('/static/data/sdm/ne_10m_land.geojson').then(function (r) { return r.json(); }).then(function (geo) {
          if (!map.getPane('landPane')) {
            map.createPane('landPane');
            map.getPane('landPane').style.zIndex = 500;
          }
          var land = L.geoJSON(geo, { pane: 'landPane', style: { color: 'none', fillColor: '#ffffff', fillOpacity: 1 } }).addTo(map);
          map._farmSeasonLand = land;
