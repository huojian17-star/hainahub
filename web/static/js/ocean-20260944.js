...
(function () {
  ...
  function showPost(post) {
    ...
    detail.innerHTML =
      ...
      (post.address ? '<p class="post-addr"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="vertical-align:-2px;margin-right:4px;"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>' + esc(post.address) + '</p>' : "") +
  ...
  function switchMode(mode) {
    currentMode = mode;
    if (window.sdmSetVisible) window.sdmSetVisible(mode === "species");
    ...
    if (mode === "tide") {
      ...
      if (window.sdmSetVisible) window.sdmSetVisible(true);
      document.getElementById("mapTip").style.display = "none";
      document.getElementById("sstLegend").classList.add("hidden");
      document.getElementById("chlLegend").classList.add("hidden");
      document.getElementById("sssLegend").classList.add("hidden");
  ...
  // SST 自动播放（时间滑块动画，借鉴 GeoLibre TimeSlider）
  var sstPlayTimer = null;
  var sstPlayBtn = document.getElementById("sstPlayBtn");
  function sstSetOffset(off) {
    if (sstTimeRange) sstTimeRange.value = String(off);
    if (sstTimeLabel) {
      if (off === 0) sstTimeLabel.textContent = "当前";
      else if (off < 0) sstTimeLabel.textContent = off + "h";
      else sstTimeLabel.textContent = "+" + off + "h";
    }
    if (currentMode === "sst") loadSstLayer(sstTimeStr(off));
  }
  if (sstPlayBtn) {
    sstPlayBtn.addEventListener("click", function () {
      if (sstPlayTimer) {
        clearInterval(sstPlayTimer);
        sstPlayTimer = null;
        sstPlayBtn.classList.remove("playing");
        sstPlayBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M8 5v14l11-7z"/></svg>';
        return;
      }
      sstPlayBtn.classList.add("playing");
      sstPlayBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M6 5h4v14H6zM14 5h4v14h-4z"/></svg>';
      var cur = sstTimeRange ? parseInt(sstTimeRange.value, 10) : 0;
      if (cur >= 168) cur = -24;
      sstSetOffset(cur);
      sstPlayTimer = setInterval(function () {
        var off = sstTimeRange ? parseInt(sstTimeRange.value, 10) : 0;
        off += 1;
        if (off > 168) off = -24;  // 循环
        sstSetOffset(off);
      }, 1200);
    });
  }
  ...
  function loadBathyLayer() {
    ...
    // 点击地图查水深（只在 bathy 模式绑定一次，避免重复）
    if (!bathyClickBound) {
      bathyClickBound = true;
      map.on("click", function (e) {
        if (currentMode !== "bathy") return;
        var lat = e.latlng.lat;
        var lon = e.latlng.lng;
        // 在点击位置弹出气泡显示水深（用户点哪里就显示在哪里）
        var popup = L.popup({ closeButton: true, autoPan: true, maxWidth: 260 })
          .setLatLng(e.latlng)
          .setContent('正在查询 ' + lat.toFixed(2) + '°N/' + lon.toFixed(2) + '°E 的水深…')
          .openOn(map);
        fetch("/api/ocean-depth?lat=" + lat + "&lon=" + lon).then(function (r) { return r.json(); }).then(function (d) {
          if (!d.ok) {
            popup.setContent('该点无水深数据');
            return;
          }
          var depth = d.depth;
          var depthText = depth < 0 ? (-depth).toFixed(1) + ' 米' : depth.toFixed(1) + ' 米(陆地)';
          popup.setContent(
            '<div class="depth-popup">' +
            '<div class="depth-popup-title">水深</div>' +
            '<div class="depth-popup-val">' + depthText + '</div>' +
            '<div class="depth-popup-coord">' + d.lat.toFixed(2) + '°N, ' + d.lon.toFixed(2) + '°E</div>' +
            '<div class="depth-popup-src">数据：<a href="https://www.ncei.noaa.gov/products/etopo-global-relief-model" target="_blank" rel="noopener">NOAA NCEI ETOPO 2022</a></div>' +
            '</div>'
          );
        }).catch(function () {
          popup.setContent('水深查询失败，请稍后重试');
        });
      });
    }
  }
  var bathyClickBound = false;
  // ============ 海洋生物图鉴（WoRMS + Wikimedia + OBIS） ============
  var speciesData = null;  // 物种列表缓存
  var speciesLoaded = false;
  var speciesCurDetail = null;  // 当前查看详情的物种
  var cascaderPath = [];  // 级联选择当前路径，如 ["Chordata", "Mammalia", "Cetartiodactyla"]
  var treeRanks = ["phylum", "class", "ordr", "family"];  // 分类层级字段顺序
  var SPECIES_PAGE_SIZE = 30;  // 每页物种卡片数
  var speciesPage = 1;  // 当前分页页码
  var filteredSpecies = [];  // 级联+搜索过滤后的完整列表（用于分页）
  ...
  function renderSpeciesList(list) {
    ...
    if (!listEl) return;
    // 级联 + 搜索叠加过滤
    filteredSpecies = getFilteredSpecies(list);
    speciesPage = 1;
    renderSpeciesPage(listEl);
    renderCascader();
  }
  // 级联路径 + 搜索词过滤
  function getFilteredSpecies(list) {
    var q = (document.getElementById("speciesSearch") ? document.getElementById("speciesSearch").value : "").trim().toLowerCase();
    return list.filter(function (s) {
      // 级联前缀匹配：cascaderPath 第 i 项必须匹配 s[treeRanks[i]]
      for (var i = 0; i < cascaderPath.length; i++) {
        if (s[treeRanks[i]] !== cascaderPath[i]) return false;
      }
      // 搜索词匹配
      if (q) {
  ...
  // 渲染当前页（前 speciesPage * PAGE_SIZE 个）
  function renderSpeciesPage(listEl) {
    if (!filteredSpecies.length) { listEl.innerHTML = '<p class="species-loading">未找到匹配物种</p>'; return; }
    var end = Math.min(speciesPage * SPECIES_PAGE_SIZE, filteredSpecies.length);
    var html = filteredSpecies.slice(0, end).map(renderSpeciesCard).join("");
    if (end < filteredSpecies.length) html += '<div class="species-load-more" id="speciesLoadMore">加载更多…</div>';
  ...
  function loadMoreSpecies() {
    speciesPage++;
    var listEl = document.getElementById("speciesList");
    if (listEl) renderSpeciesPage(listEl);
  }
  // 层级标签
  function rankLabel(rank) {
    if (rank === "phylum") return "门";
    if (rank === "class") return "纲";
    if (rank === "ordr") return "目";
    return "科";
  }
  // 级联选择器：根据 cascaderPath 刷新 4 个下拉框选项（联动）
  function renderCascader() {
    var rankSel = ["cascaderPhylum", "cascaderClass", "cascaderOrdr", "cascaderFamily"];
    for (var i = 0; i < rankSel.length; i++) {
      var sel = document.getElementById(rankSel[i]);
      if (!sel) continue;
      var rank = treeRanks[i];
      // 第 i 层是否启用：第 0 层（门）始终启用，其余需 cascaderPath 前 i 项已确定
      var enabled = (i === 0) || (cascaderPath.length >= i);
      if (!enabled) {
        sel.disabled = true;
        sel.innerHTML = '<option value="">' + rankLabel(rank) + '（全部）</option>';
        continue;
      }
      // 候选：cascaderPath 前 i 项确定的子分类（带计数）
      var opts = {};
      var prefix = cascaderPath.slice(0, i);
      speciesData.forEach(function (s) {
        var ok = true;
        for (var j = 0; j < prefix.length; j++) {
          if (s[treeRanks[j]] !== prefix[j]) { ok = false; break; }
        }
        if (!ok) return;
        var val = s[rank];
        if (val) opts[val] = (opts[val] || 0) + 1;
      });
      var names = Object.keys(opts).sort();
      var curVal = cascaderPath[i] || "";
      var html = '<option value="">' + rankLabel(rank) + '（全部）</option>';
      names.forEach(function (n) {
        var cn = taxCn(rank, n);
        var label = (cn !== n) ? cn + " " + n : n;
        html += '<option value="' + esc(n) + '"' + (n === curVal ? ' selected' : '') + '>' + esc(label) + '（' + opts[n] + '）</option>';
      });
      sel.innerHTML = html;
      sel.disabled = false;
    }
  ...
  function renderSpeciesCard(s) {
    ...
    var taxHtml = '';
    if (s.phylum_cn) taxHtml += esc(s.phylum_cn);
    if (s.class_cn) taxHtml += (taxHtml ? ' / ' : '') + esc(s.class_cn);
    if (!taxHtml && s.phylum) taxHtml = esc(s.phylum);
    return '<div class="species-card" data-id="' + s.id + '">' +
      img +
      '<div class="species-card-body">' +
      nameHtml +
      '<div class="species-card-tax">' + taxHtml + '</div>' +
      '<div class="species-card-obis">OBIS 记录：' + (s.obis_total || 0) + '</div>' +
      '</div></div>';
  }
  // 分类名转中文（从 speciesData 构建映射：每个层级的学名→中文名）
  function taxCn(rank, name) {
    if (!name) return name;
    if (!speciesData) return name;
    var cnField = rank + "_cn";
    for (var i = 0; i < speciesData.length; i++) {
      if (speciesData[i][rank] === name && speciesData[i][cnField]) {
        return speciesData[i][cnField];
      }
    }
    return name;
  }
  ...
  function showSpeciesDetail(id) {
    ...
    fetch("/api/species/" + id).then(function (r) { return r.json(); }).then(function (d) {
      ...
      bodyEl.innerHTML =
        ...
        (s.desc ? '<div class="species-detail-desc">' + esc(s.desc) + '</div>' : '') +
        (s.desc_url ? '<div class="species-detail-src">内容来源：<a href="' + esc(s.desc_url) + '" target="_blank" rel="noopener">维基百科</a>（CC BY-SA 4.0）· 仅供参考</div>' : '') +
  ...
  function showSpeciesDistribution(id) {
    ...
    if (!s) return;
    // 2026-08-30：按钮加 loading 状态，防止用户误以为没点到而狂按（OBIS 查询约 5s）
    var distBtn = document.getElementById("speciesDistBtn");
    var btnOriginalText = distBtn ? distBtn.innerHTML : "";
    if (distBtn) {
      distBtn.disabled = true;
      distBtn.classList.add("loading");
      distBtn.innerHTML = '<span class="dist-spinner"></span> 加载中…';
    }
    ...
    function restoreBtn() {
      if (distBtn) {
        ...
        distBtn.classList.remove("loading");
        distBtn.innerHTML = btnOriginalText;
      }
    }
    fetch("/api/species/" + id + "/distribution").then(function (r) { return r.json(); }).then(function (d) {
      ...
      var points = d.points || [];
      if (!points.length) { fs.innerHTML = '分布数据：该物种暂无分布记录'; restoreBtn(); return; }
  ...
  // 级联选择器 change 事件（联动：选完一级刷新下一级 + 过滤列表）
  var cascaderSelects = ["cascaderPhylum", "cascaderClass", "cascaderOrdr", "cascaderFamily"];
  cascaderSelects.forEach(function (id, i) {
    var sel = document.getElementById(id);
    if (!sel) return;
    sel.addEventListener("change", function () {
      var val = sel.value;
      // 截断 cascaderPath 到 i，再设置第 i 项（清空则只保留前 i 项）
      cascaderPath = cascaderPath.slice(0, i);
      if (val) cascaderPath[i] = val;
      if (speciesData) renderSpeciesList(speciesData);
    });
  });
  // 物种列表：卡片点击 + 加载更多
  ...
  if (speciesList) speciesList.addEventListener("click", function (e) {
    // 加载更多点击
    var more = e.target.closest ? e.target.closest("#speciesLoadMore") : null;
    if (more) { loadMoreSpecies(); return; }
    // 物种卡片点击：显示详情
  ...
  if (speciesPanel) speciesPanel.addEventListener("scroll", function () {
    if (!filteredSpecies.length) return;
    if (speciesPanel.scrollTop + speciesPanel.clientHeight >= speciesPanel.scrollHeight - 60) {
      if (speciesPage * SPECIES_PAGE_SIZE < filteredSpecies.length) loadMoreSpecies();
    }
  });
  if (chlTimeSelect) {
    initChlDates();
