...
(function () {
  ...
  function loadBathyLayer() {
    ...
    if (bathyLoading) {
      ...
      bathyLoadingTimer = setTimeout(function () { bathyLoading.classList.add("hidden"); }, 15000);
    }
    // 走后端代理（NOAA ERDDAP 未开 CORS，浏览器直接请求被拦截）
  ...
  function renderSpeciesList(list) {
    ...
    var html = filtered.map(function (s) {
      ...
      return '<div class="species-card" data-id="' + s.id + '">' +
        ...
        '<div class="species-card-name">' + esc(s.scientificname) + '</div>' +
  ...
  function showSpeciesDetail(id) {
    ...
    fetch("/api/species/" + id).then(function (r) { return r.json(); }).then(function (d) {
      ...
      detailEl.innerHTML =
        '<button class="species-detail-back" id="speciesDetailBack">← 返回列表</button>' +
        '<div class="species-detail-head">' +
        '<div class="species-detail-name">' + esc(s.scientificname) + '</div>' +
        (s.authority ? '<div class="species-detail-authority">' + esc(s.authority) + '</div>' : '') +
        '</div>' +
        img +
      ...
      var back = document.getElementById("speciesDetailBack");
      if (back) back.addEventListener("click", function () {
  ...
  function showSpeciesDistribution(id) {
    ...
    fetch("/api/species/" + id + "/distribution").then(function (r) { return r.json(); }).then(function (d) {
      ...
      var points = d.points || [];
      if (!points.length) { alert("该物种暂无分布记录"); return; }
      ...
      var fs = document.getElementById("flowSource");
      fs.classList.remove("hidden");
      fs.innerHTML = '分布数据：<a href="https://obis.org/" target="_blank" rel="noopener">OBIS</a>（' + esc(s.scientificname) + '，' + (d.total || 0) + ' 条记录）· 仅供参考';
    });
  }
