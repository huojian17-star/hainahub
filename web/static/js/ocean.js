...
(function () {
  ...
  // ========== 台风（星图，活跃台风标记 + 路径/信息）==========
  var typhoonMarkers = [];
  var typhoonRouteLayer = null;
  var typhoonInfoLayer = null;
  function clearTyphoon() {
    typhoonMarkers.forEach(function (m) { if (m._map) map.removeLayer(m); });
    typhoonMarkers = [];
    if (typhoonRouteLayer && typhoonRouteLayer._map) map.removeLayer(typhoonRouteLayer);
    if (typhoonInfoLayer && typhoonInfoLayer._map) map.removeLayer(typhoonInfoLayer);
    typhoonRouteLayer = null;
    typhoonInfoLayer = null;
  }
  function loadTyphoon() {
    clearTyphoon();
    var perf = window.PerfMode ? window.PerfMode.getMode() : "quality";
    // 台风标记轻量，不因 perf=lite 跳过（保证台风始终可见）
    fetch("/api/typhoon/list")
      .then(function (r) { return r.json(); })
      .then(function (data) {
        if (!data.ok || !data.typhoons || !data.typhoons.length) {
          console.warn("无活跃台风", data);
          return;
        }
        // 先 fetch 所有台风详情（拿实际位置），再创建标记（避免先放 0,0 闪烁/偏位）
        var detailPromises = data.typhoons.map(function (t) {
          return fetch("/api/typhoon/detail?tpId=" + t.tpId)
            .then(function (r) { return r.json(); })
            .then(function (d) {
              if (d.ok && d.typhoon) { t._detail = d.typhoon; }
              return t;
            })
      ...
      .then(function (typhoons) {
        typhoons.forEach(function (t) {
          var detail = t._detail;
          var rt = detail ? (detail.rtPoints || []) : [];
          var last = rt.length ? rt[rt.length - 1] : null;
          if (!last) { return; }  // 无实况点不显示
          var icon = L.divIcon({
            className: "typhoon-icon",
            html: '<span class="typhoon-dot"></span>',
            iconSize: [18, 18],
            iconAnchor: [9, 9]
          });
          var marker = L.marker([last.centerLat, last.centerLon], { icon: icon }).addTo(map);
          marker._tpId = t.tpId;
          marker._tName = t.name;
          marker.bindPopup(typhoonPopupHtml(detail, t.tpId));
          typhoonMarkers.push(marker);
        });
        // 有台风则适配视野到台风区域（单点用 setView，多点用 fitBounds）
        if (typhoonMarkers.length === 1) {
          var ll = typhoonMarkers[0].getLatLng();
          map.setView([ll.lat, ll.lng], 5, { animate: false });
        } else if (typhoonMarkers.length > 1) {
          var bounds = L.latLngBounds(typhoonMarkers.map(function (m) { return m.getLatLng(); }));
          map.fitBounds(bounds, { padding: [60, 60] });
        }
      })
      .catch(function (e) { console.warn("台风列表失败", e); });
  }
  function typhoonPopupHtml(typhoon, tpId) {
    ...
    return '<div class="typhoon-popup">' +
      ...
      '<div class="tp-btn" data-tpid="' + tpId + '">查看路径</div>' +
    '</div>';
  }
  // 事件委托：点「查看路径」显示台风路径
  document.addEventListener("click", function (e) {
    ...
    if (!btn) return;
    var tpId = btn.getAttribute("data-tpid");
    showTyphoonRoute(tpId);
  });
  function showTyphoonRoute(tpId) {
    if (typhoonRouteLayer && typhoonRouteLayer._map) map.removeLayer(typhoonRouteLayer);
    if (typhoonInfoLayer && typhoonInfoLayer._map) map.removeLayer(typhoonInfoLayer);
    fetch("/api/typhoon/detail?tpId=" + tpId)
      ...
      .then(function (d) {
        ...
        // 实况路径（实线）
        if (rtLatLngs.length) {
          typhoonRouteLayer = L.polyline(rtLatLngs, { color: "#ff6600", weight: 3, opacity: 0.9 }).addTo(map);
        }
        // 预报路径（虚线）
        if (fcLatLngs.length) {
          typhoonInfoLayer = L.polyline(fcLatLngs, { color: "#ff3366", weight: 3, opacity: 0.9, dashArray: "6 6" }).addTo(map);
        }
        // 适配到路径范围
        if (rtLatLngs.length) {
          map.fitBounds(L.latLngBounds(rtLatLngs.concat(fcLatLngs)), { padding: [40, 40] });
        }
        // 显示路径说明
        var fs = document.getElementById("flowSource");
        fs.classList.remove("hidden");
        fs.innerHTML = '台风路径：<span class="tp-legend-rt">实线=实况</span> <span class="tp-legend-fc">虚线=预报</span> · 数据来自 <a href="https://datacloud.geovisearth.com/" target="_blank" rel="noopener">星图地球数据云</a>';
      })
