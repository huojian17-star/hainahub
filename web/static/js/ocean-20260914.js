      ...
      .then(function (d) {
        ...
        if (fcArr && fcArr.length) {
          fcArr.forEach(function (p, idx) {
            ...
            // 起点（第一个预报点）用红点，其他用青色 + 时间 label
            var isFirst = idx === 0;
            var dotClass = isFirst ? "typhoon-fc-dot-first" : "typhoon-fc-dot";
            ...
            m.bindTooltip((p.fc_time || "") + " 到达", { direction: "top", offset: [0, -8] });
            // 标注时间（短格式，如 27日17时）——常显 tooltip，不靠 hover
            if (p.fc_time && p.fc_time.length >= 16) {
              var tstr = p.fc_time.slice(5, 7) + "日" + p.fc_time.slice(11, 13) + "时";
              m.bindTooltip(tstr, { direction: "top", offset: [0, -10], permanent: true, className: "typhoon-fc-label" });
            }
