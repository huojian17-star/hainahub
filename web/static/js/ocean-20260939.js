  ...
  function loadBathyLayer() {
    ...
    if (!bathyClickBound) {
      ...
      map.on("click", function (e) {
        ...
        var fs = document.getElementById("flowSource");
        fs.classList.remove("hidden");
        fs.innerHTML = '正在查询 ' + lat.toFixed(2) + '°N/' + lon.toFixed(2) + '°E 的水深…';
        fetch("/api/ocean-depth?lat=" + lat + "&lon=" + lon).then(function (r) { return r.json(); }).then(function (d) {
          if (!d.ok) {
            fs.innerHTML = '该点无水深数据';
            return;
          }
          var depth = d.depth;
          var depthText = depth < 0 ? (-depth).toFixed(1) + ' 米' : depth.toFixed(1) + ' 米(陆地)';
          fs.innerHTML = '水深：' + depthText + '（' + d.lat.toFixed(2) + '°N, ' + d.lon.toFixed(2) + '°E）· <a href="https://www.ncei.noaa.gov/products/etopo-global-relief-model" target="_blank" rel="noopener">NOAA NCEI ETOPO 2022</a>';
        }).catch(function () {
          fs.innerHTML = '水深查询失败，请稍后重试';
        });
      });
    }
  }
