...
(function () {
  ...
  var treePath = [];  // 分类树当前展开路径，如 ["Chordata", "Mammalia", "Cetartiodactyla"]
  ...
  function renderSpeciesList(list) {
    ...
    if (q) {
      ...
      listEl.innerHTML = filtered.map(renderSpeciesCard).join("");
      return;
    }
    // 分类树模式：根据 treePath 逐级下钻
    renderTree(list, listEl);
  }
  ...
  function renderTree(list, listEl) {
    // 面包屑（当前路径，可点击返回；显示中文名）
    var crumbs = '<div class="species-crumbs">';
    if (treePath.length) {
      crumbs += '<span class="species-crumb" data-level="-1"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg></span>';
      crumbs += treePath.map(function (name, i) {
        return '<span class="species-crumb-sep">/</span><span class="species-crumb" data-level="' + i + '">' + esc(taxCn(treeRanks[i], name)) + '</span>';
      }).join("");
    }
    crumbs += '</div>';
    // 当前要显示的层级
    var level = treePath.length;
    // 过滤出当前路径下的物种
    var inPath = list.filter(function (s) {
      return treePath.every(function (name, i) {
        if (name === "__unclassified__") return !s[treeRanks[i]];
        return s[treeRanks[i]] === name;
      });
    });
    if (level >= treeRanks.length) {
      // 到了科层级，显示物种卡片
      if (!inPath.length) { listEl.innerHTML = crumbs + '<p class="species-loading">该分类下暂无物种</p>'; return; }
      listEl.innerHTML = crumbs + inPath.map(renderSpeciesCard).join("");
      return;
    }
    // 显示当前层级的子分类（去重 + 计数）
    var rank = treeRanks[level];
    var children = {};
    var noRankCount = 0;
    inPath.forEach(function (s) {
      var val = s[rank];
      if (val) children[val] = (children[val] || 0) + 1;
      else noRankCount++;
    });
    var childNames = Object.keys(children).sort();
    // 字段为空的物种归入「未分类」节点，避免从分类树消失（保证计数与实际内容一致）
    if (noRankCount > 0) {
      children["__unclassified__"] = noRankCount;
      childNames.push("__unclassified__");
    }
    // 若当前层级只有「未分类」子节点（即该层无真实子分类，如目字段缺失），直接显示物种卡片
    if (childNames.length === 1 && childNames[0] === "__unclassified__") {
      if (inPath.length) { listEl.innerHTML = crumbs + inPath.map(renderSpeciesCard).join(""); return; }
    }
    if (!childNames.length) {
      // 当前层级无子分类（如目字段缺失），但确有物种 → 直接显示物种卡片，不再误报「暂无物种」
      if (inPath.length) { listEl.innerHTML = crumbs + inPath.map(renderSpeciesCard).join(""); return; }
      listEl.innerHTML = crumbs + '<p class="species-loading">该分类下暂无物种</p>'; return;
    }
    var html = crumbs + childNames.map(function (name) {
      if (name === "__unclassified__") {
        return '<div class="species-tree-node" data-name="__unclassified__">' +
          '<span class="species-tree-arrow">▸</span>' +
          '<span class="species-tree-name">未分类</span>' +
          '<span class="species-tree-count">' + children[name] + '</span></div>';
      }
      var cn = taxCn(rank, name);
      // 中文名 + 英文学名小字（有中文名才显示英文，否则只显示英文）
      var nameHtml = (cn !== name) ? '<span class="species-tree-name">' + esc(cn) + '<span class="species-tree-sci">' + esc(name) + '</span></span>' : '<span class="species-tree-name">' + esc(name) + '</span>';
      return '<div class="species-tree-node" data-name="' + esc(name) + '">' +
        '<span class="species-tree-arrow">▸</span>' +
        nameHtml +
        '<span class="species-tree-count">' + children[name] + '</span></div>';
    }).join("");
  ...
  if (speciesList) speciesList.addEventListener("click", function (e) {
    // 分类树节点点击：下钻（展开下一级）
    var node = e.target.closest ? e.target.closest(".species-tree-node") : null;
    if (node) {
    ...
    // 面包屑点击：返回上一级
    var crumb = e.target.closest ? e.target.closest(".species-crumb") : null;
    if (crumb) {
      var level = parseInt(crumb.getAttribute("data-level"), 10);
      treePath = treePath.slice(0, level + 1);  // 截断到点击的层级
      renderSpeciesList(speciesData);
      return;
    }
    // 物种卡片点击：显示详情
