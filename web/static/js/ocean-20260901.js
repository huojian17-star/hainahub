...
(function () {
    ...
    fetch("/api/nsmc-cloud-time")
      ...
      .then(function (data) {
        ...
        if (cloudRange) {
          ...
          cloudRange.value = cloudTimes.length - 1;  // 默认最新
        }
  ...
  function showCloudFrame(idx) {
    ...
    // 清除旧图层，加载新云图
    if (cloudLayer && cloudLayer._map) map.removeLayer(cloudLayer);
    // bbox 中国附近（经纬度），imageOverlay 叠加
    var bounds = L.latLngBounds([0, 90], [45, 135]);
    ...
    if (cloudInfo) {
      ...
      cloudInfo.innerHTML = '<div class="tt-time">' + ds + ' ' + ts.slice(0, 2) + ':' + ts.slice(2, 4) + ' UTC</div>';
    }
  }
  // 云图时间轴滑块事件
  if (cloudRange) {
