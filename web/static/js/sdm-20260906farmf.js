...
function loadFarmDetail(el) {
  ...
  fetch('/api/ocean-water/alert?lat=' + lat + '&lon=' + lon)
    ...
    .then(function (d) {
      ...
      var chl = '—（近10日双星无晴观测）';
      if (data.chl && data.chl.value != null) {
        chl = data.chl.value.toFixed(2) + ' mg/m³';
        var chlD = (data.chl.time || '').slice(5, 10);
        if (chlD) chl += '（' + chlD.replace('-', '月') + '日' + (data.chl.source ? ' · ' + data.chl.source : '') + (data.chl.n > 1 ? ' n=' + data.chl.n : '') + '）';
        else if (data.chl.source) chl += '（' + data.chl.source + '）';
      }
...
function showFarmSeason() {
  ...
  map._farmSeasonReq = (map._farmSeasonReq || 0) + 1;
  var myReq = map._farmSeasonReq;
  ...
  fetch('/static/data/sdm/sdm_prediction_' + sp + '_' + season + '_cn.json')
    .then(function (r) { return r.json(); })
    .then(function (d) {
      if (map._farmSeasonReq !== myReq) return;  // 过期响应，丢弃
